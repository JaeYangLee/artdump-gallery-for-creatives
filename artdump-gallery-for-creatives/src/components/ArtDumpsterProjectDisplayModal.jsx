...
import ArtDumpsterProjectSettingsModal from "../components/ArtDumpsterProjectSettingsModal";

function ArtDumpsterProjectDisplayModal({
  isProjectDisplayModalOpen,
  onProjectDisplayModalClose,
}) {
  if (!isProjectDisplayModalOpen) return null;
  const [isProjectSettingsModalOpen, setIsProjectSettingsOpen] =
    useState(false);
  ...
  return (
    <>
      {/* body */}
      <div
        onClick={onProjectDisplayModalClose}
      ...
      >
        ...
        <AnimatePresence>
          ...
          <motion.div
            ...
            className="flex flex-row bg-[#F8FAFC] w-[1350px] h-[620px]"
              ...
              <div className="p-4 flex justify-center items-center ">
                <div className="p-4 border border-[#372F3D]/40 rounded-2xl w-[400px] h-[380px] font-barlow">
                  <p className="font-light italic text-[#372F3D] text-[14px]">
                    "This digital portrait captures a quiet, introspective
                    moment, inspired by soft afternoon light and muted tones. I
                    focused on rendering subtle facial expressions and delicate
                    brush strokes to give the piece an emotional depth. Created
                    using Procreate, this artwork was part of my ongoing study
                    in lighting and facial anatomy. I experimented with textured
                    brushes to add a painterly feel while keeping the overall
      ...
      <ArtDumpsterProjectSettingsModal
        isProjectSettingsModalOpen={isProjectSettingsModalOpen}
        onProjectSettingsModalClose={() => setIsProjectSettingsOpen(false)}
      />
    </>
  );
}
